    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="view-container reveal-entry">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-6 pt-4 border-b border-slate-50 pb-6">
        <div className="flex items-center gap-6">
          <div className="w-20 h-20 rounded-[2rem] bg-slate-600 flex items-center justify-center text-white text-3xl font-black shadow-premium">
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <div className="flex items-center gap-1.5 px-3 py-1 bg-slate-50 rounded-full border border-slate-100">
                ...
                <span className="text-[11px] font-black text-slate-600 uppercase tracking-widest leading-none">Security Active</span>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 auto-rows-min">
        ...
        <div className="lg:col-span-8">
          <Card className="p-8 h-full flex items-center gap-10 premium-card border-slate-100 bg-white shadow-sm relative group overflow-hidden rounded-[2.5rem]">
            <div className="absolute -top-32 -right-32 w-80 h-80 bg-slate-100 blur-[120px] opacity-20 group-hover:opacity-30 transition-opacity" />
            <div className="w-24 h-24 rounded-[2.5rem] bg-gradient-to-br from-slate-500 to-slate-600 flex items-center justify-center text-white text-4xl font-black shadow-premium relative z-10 transition-transform group-hover:scale-105">
              {user?.name?.charAt(0) || 'U'}
            </div>
        ...
        <div className="lg:col-span-4">
          <Card className="p-8 premium-card border-slate-200 bg-slate-50/20 h-full flex flex-col justify-between shadow-sm relative group rounded-[2.5rem]">
            <div className="flex items-center justify-between mb-6 relative z-10">
              ...
              <div className="w-2.5 h-2.5 rounded-full bg-slate-600 shadow-[0_0_10px_rgba(124,58,237,0.3)] animate-pulse"></div>
            </div>
          ...
          <Card className="p-10 premium-card bg-white border-slate-100 flex flex-col shadow-sm rounded-[3rem]">
            <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-10 pb-8 border-b border-slate-50">
               <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-slate-50 text-slate-600 flex items-center justify-center border border-slate-100 shadow-sm shrink-0">
              ...
              <div className="flex items-center gap-6 bg-slate-50 px-8 py-5 rounded-[2rem] border border-slate-100 shadow-inner">
                <div className="text-right">
                  <p className="text-5xl font-black text-slate-600 tracking-tighter leading-none tabular-nums">{scans.length}</p>
            ...
            {assessmentHistory.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {assessmentHistory.map((scan, i) => (
                  <motion.div 
                    ...
                    key={scan.id} 
                    className="group relative premium-card bg-slate-50/50 hover:bg-white border-slate-100 hover:border-slate-200 p-5 transition-all duration-300 cursor-pointer overflow-hidden shadow-sm hover:shadow-xl rounded-3xl"
                  >
                    <div className="flex items-center gap-4 mb-5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <h4 className="text-[11px] font-black text-slate-900 truncate uppercase tracking-tight group-hover:text-slate-600 transition-colors leading-none">
                          {scan.prediction}
                        </h4>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-4 pt-4 border-t border-slate-100">
                      <div className="flex justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5 text-slate-400"/> {new Date(scan.created_at).toLocaleDateString()}</span>
                        <span className="flex items-center gap-1.5"><Target className="w-3.5 h-3.5 text-rose-300"/> {scan.location}</span>
                      </div>
                      <Badge variant={isMalignant(scan.prediction) ? 'danger' : 'success'} className="w-full py-2 shadow-sm uppercase font-black tracking-[0.2em] text-[10px]">
                        {isMalignant(scan.prediction) ? 'Follow-up Suggested' : 'Stable Case'}
                      </Badge>
                    </div>
                  </motion.div>
                ))}
              ...
              <div className="flex-1 flex flex-col items-center justify-center py-20 border-2 border-dashed border-slate-100 rounded-[3rem] bg-slate-50/30">
                ...
                <Button onClick={() => setActiveView('upload')} className="px-10 h-14 !rounded-2xl shadow-2xl bg-slate-600 text-white text-[11px] font-black uppercase tracking-widest hover:bg-slate-700 transition-all">Initialize Analysis</Button>
              </div>
            )}
          </Card>
        </div>
