    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="view-container reveal-entry">
      <div className="max-w-4xl mx-auto w-full">
        <header className="flex items-center gap-4 mb-8">
            <div className="w-12 h-12 rounded-xl bg-violet-100 flex items-center justify-center text-violet-700 shadow-sm">
        ...
        <div className="space-y-6">
           {/* Profile Identity */}
         <Card className={`p-6 bg-slate-50 border border-slate-100 rounded-3xl transition-all ${isEditing ? 'ring-2 ring-violet-500/20 shadow-inner bg-white' : ''}`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4 w-full">
                 ...
                 {!isEditing ? (
                   ...
                   <div className="space-y-2 w-full animate-in fade-in slide-in-from-left-1 transition-all">
                      <input 
                        className="bg-white border border-slate-200 rounded-xl px-4 py-2 text-sm font-medium text-slate-900 w-full focus:outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-200 transition-all" 
                        value={formData.name}
              ...
              <div className="ml-4">
                {isEditing ? (
                  <Button onClick={handleSave} disabled={isSaving} className="!rounded-xl px-6 h-11 bg-violet-600 text-white hover:bg-violet-700 flex items-center shadow-md">
                    {isSaving ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}
                    Save
                  </Button>
                ) : (
                  <Button onClick={() => setIsEditing(true)} variant="outline" className="!rounded-xl h-11 px-6 border-slate-200 text-sm font-semibold text-slate-600 hover:text-violet-700 hover:bg-violet-50 transition-all">Edit Profile</Button>
                )}
              </div>
            </div>
         </Card>
...
function SettingsItem({ icon: IconComponent, label, desc, active, onClick }) {
  return (
    <div 
        onClick={onClick}
        className={`flex items-center justify-between p-4 rounded-xl transition-all cursor-pointer group border ${active ? 'bg-violet-50/50 border-violet-200' : 'bg-white border-slate-200 hover:border-violet-300'}`}
    >
       <div className="flex items-center gap-4">
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all ${active ? 'bg-white text-violet-600 shadow-sm border border-violet-100' : 'bg-slate-50 text-slate-400 border border-slate-100 group-hover:text-violet-500'}`}>
             {IconComponent && <IconComponent className="w-5 h-5" />}
          </div>
          <div>
            <p className={`text-sm font-semibold mb-1 ${active ? 'text-violet-800' : 'text-slate-900'}`}>{label}</p>
            <p className="text-xs font-medium text-slate-500">{desc}</p>
          </div>
       </div>
       <div className={`w-11 h-6 rounded-full transition-all relative p-1 ${active ? 'bg-violet-600' : 'bg-slate-200'}`}>
          <motion.div 
